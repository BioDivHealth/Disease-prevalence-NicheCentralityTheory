...
backgroundPOINTS<-function(presence, # [sf points] distribution points of the species
                           ...
                           weights.p="Random"  # [Character]["Random","BwData"] Method to draw the background points: BwData, background weighted data, a density kernell is first calculated and 
   ...
   pack<-c("dplyr","MASS","sf","classInt","terra")
   ...
   if(!is.null(range_samp)){
      ...
      ifelse(is.character(range_samp),
   ...
   if(weights.p=="BwData"){
      ...
      yy<-ifelse(is.null(cut_area),NULL,FALSE)
